...
async def test_mem_top(dut):
    ...
    dut._log.info(f"addr phase: ui_in={int(dut.ui_in.value):#04x} uio_in={int(dut.uio_in.value):#04x}")

    # data phase  
    await RisingEdge(dut.clk)
    await Timer(2, units="ns")
    dut.ui_in.value  = 0x03
    dut.uio_in.value = (1<<5)|(1<<4)|(1<<3)
    await Timer(1, units="ns")
    dut._log.info(f"data phase: ui_in={int(dut.ui_in.value):#04x} uio_in={int(dut.uio_in.value):#04x}")
