...
async def cpu_write(dut, addr, wdata):
    ...
    # Wait for write ack with cycle counter to detect hangs
...
async def cpu_read(dut, addr):
    ...
    # Wait for read response with cycle counter to detect hangs
    for cycle in range(200):
        await RisingEdge(dut.clk)
        await Timer(2, unit="ns")
        resp_data, resp_valid, resp_rw = get_resp(dut)
...
async def test_mem_top(dut):
    ...
    # Reset for 5 cycles to be safe with gate-level delays
    dut.rst_n.value = 0
    for _ in range(5):
        await RisingEdge(dut.clk)
    await Timer(2, unit="ns")
