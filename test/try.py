...
async def cpu_write(dut, addr, wdata):
    ...
    for cycle in range(500):
        ...
        await Timer(2, unit="ns")
        _, resp_valid, resp_rw = get_resp(dut)
        dut._log.info(f"  cpu_write poll [{cycle}]: uio_out={int(dut.uio_out.value):08b} valid={resp_valid} rw={resp_rw}")
        if resp_valid and resp_rw:
            return
...
async def cpu_read(dut, addr):
    ...
    for cycle in range(500):
        ...
        await Timer(2, unit="ns")
        resp_data, resp_valid, resp_rw = get_resp(dut)
        dut._log.info(f"  cpu_read poll [{cycle}]: uio_out={int(dut.uio_out.value):08b} valid={resp_valid} rw={resp_rw} data=0x{resp_data:02X}")
        if resp_valid and not resp_rw:
            return resp_data
...
async def test_mem_top(dut):
    ...
    # Extra settling cycles for GL — sky130 cells need time to initialise
    for _ in range(20):
        await RisingEdge(dut.clk)
    await Timer(2, unit="ns")
